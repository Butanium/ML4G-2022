...
for batch in tqdm(batches[:nb_batch]):
    ...
    with torch.no_grad():
        ...
        for i, partial_output in enumerate(hidden_states):
            try:
                data_couche_n[i].append(partial_output.cpu())
            except:
                data_couche_n.append([partial_output.cpu()])
        batch.cpu()

x_n = [torch.zeros((nb_batch * batch_size, 512, 768)) for i in range(13)]
print('reshaping...')
for i in tqdm(range(13)):
    for j in range(nb_batch):
        x_n[i][j * batch_size: (j + 1) * batch_size] = data_couche_n[i][j]
    x_n[i] = torch.reshape(x_n[i][:, -1], (-1, 768)).detach()
